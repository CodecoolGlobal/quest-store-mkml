...
public class TransactionDAOSql implements TransactionDAO {
    ...
    public List<Transaction> getTransactions(int userId, int type) throws DaoException {
        String SQL = "SELECT * FROM transactions LEFT JOIN cards ON transactions.card_id = cards.id WHERE user_id = ? AND cards.card_type_id = ?";
        List<Transaction> transactionList = new ArrayList<>();
        try {
            PreparedStatement pstmt = DBCPDataSource.getConnection().prepareStatement(SQL);
            pstmt.setInt(1, userId);
            pstmt.setInt(2, type);
            ResultSet rs = pstmt.executeQuery();

            transactionService.createTransactionObject(rs);
